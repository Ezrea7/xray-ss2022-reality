...
# ============================================================
# xray_alpine_lxc_ss2022_reality.sh
# 仅保留：
#   1) Xray-core 安装/更新
...
_get_public_ip() {
    ...
    if [ -z "$ip" ]; then
        ip="$(curl -fsS6 --max-time 5 icanhazip.com 2>/dev/null || true)"
    fi
    printf '%s' "$ip"
}
...
_write_qx_template() {
    ...
    cat > "$file" <<EOF
shadowsocks=${link_ip}:${port}, method=2022-blake3-aes-256-gcm, password=${password}, obfs=over-tls, obfs-host=${sni}, tls-verification=true, reality-base64-pubkey=${REALITY_PUBLIC_KEY}, reality-hex-shortid=${REALITY_SHORT_ID}, udp-relay=true, udp-over-tcp=sp.v2, tag=${name}
EOF
}
...
_add_ss2022_reality() {
    ...
    local server_ip node_ip port sni name default_name inbound link_ip client_file password

    server_ip="$(_get_public_ip)"
    read -rp "请输入服务器 IP (默认: ${server_ip:-留空}): " node_ip
    ...
    local qx_file="${XRAY_CLIENT_DIR}/ss2022-reality-${port}-qx.conf"
    _write_qx_template "$node_ip" "$port" "$sni" "$password" "$name" "$qx_file"

    link_ip="$node_ip"
    [[ "$node_ip" == *":"* ]] && link_ip="[$node_ip]"
    local qx_link="shadowsocks=${link_ip}:${port}, method=2022-blake3-aes-256-gcm, password=${password}, obfs=over-tls, obfs-host=${sni}, tls-verification=true, reality-base64-pubkey=${REALITY_PUBLIC_KEY}, reality-hex-shortid=${REALITY_SHORT_ID}, udp-relay=true, udp-over-tcp=sp.v2, tag=${name}"

    _success "SS2022 + REALITY 节点添加成功。"
    echo -e "${YELLOW}服务器:${NC} ${link_ip}:${port}"
    ...
    echo -e "${YELLOW}Xray 客户端 JSON (UoTVersion=2):${NC} ${client_file}"
    echo -e "${YELLOW}Quantumult X (sp.v2):${NC} ${qx_link}"
    echo -e "${YELLOW}Quantumult X 配置文件:${NC} ${qx_file}"
    echo -e "${YELLOW}说明:${NC} Xray 客户端 JSON 默认使用 UoTVersion=2；圈X输出默认使用 udp-over-tcp=sp.v2。"
}
