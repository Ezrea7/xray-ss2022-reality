...
# 基础路径定义
export SCRIPT_VERSION="13-xray-ss2022-reality"
export DEFAULT_SNI="www.amd.com"
SELF_SCRIPT_PATH="$(readlink -f "$0")"
SCRIPT_DIR="$(dirname "$SELF_SCRIPT_PATH")"
SINGBOX_DIR="/usr/local/etc/sing-box"
GITHUB_RAW_BASE="https://raw.githubusercontent.com/Ezrea7/xray-ss2022-reality/refs/heads/main"
...
_uninstall() {
    ...
    _info "正在清理周边环境..."
    rm -f "${SINGBOX_DIR}/parser.sh" "${SINGBOX_DIR}/advanced_relay.sh" "${SINGBOX_DIR}/xray_alpine_lxc_ss2022_reality.sh"
    rm -f "${SCRIPT_DIR}/parser.sh" "${SCRIPT_DIR}/advanced_relay.sh" "${SCRIPT_DIR}/xray_alpine_lxc_ss2022_reality.sh"
...
_update_script() {
    ...
    # 需要更新的子脚本列表
    local sub_scripts=("advanced_relay.sh" "parser.sh" "xray_alpine_lxc_ss2022_reality.sh")
...
# 执行 Xray 核心的安装/更新 (内联实现，避免依赖 Xray 子脚本的 source)
_do_update_xray() {
    _info "--- 安装/更新 Xray 核心 ---"
...
# --- Xray 节点管理 (调用 xray_alpine_lxc_ss2022_reality.sh) ---
_xray_features() {
    # 前置检查：Xray 核心必须已安装
    if [ ! -f "/usr/local/bin/xray" ]; then
    ...
    local script_name="xray_alpine_lxc_ss2022_reality.sh"
    local script_path="${SINGBOX_DIR}/${script_name}"
